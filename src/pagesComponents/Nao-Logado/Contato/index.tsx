...
const ContactInfo = lazy(() =>
  import('./components/ContactInfo').then((mod) => ({ default: mod.ContactInfo })),
)
const ContactFormComponent = lazy(() =>
  import('./components/ContactForm').then((mod) => ({ default: mod.ContactFormComponent })),
)
const ContactFormSkeleton = lazy(() =>
  import('./components/ContactForm/ContactFormSkeleton').then((mod) => ({
    default: mod.ContactFormSkeleton,
  })),
)
...
const ContactWithSearchParams = () => {
  ...
  const { isBlocked, blockTimer, handleBlock } = useBlockTimer(
    CONTACT_CONSTANTS.BLOCK_DURATION,
    'contactBlockedUntil',
  )
  ...
  const imageProps = {
    src: '/assets/images/background/Contato.webp',
