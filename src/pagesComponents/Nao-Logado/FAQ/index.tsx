...
const CategoryTabs = lazy(() =>
  import('./components/CategoryTabs').then((mod) => ({ default: mod.CategoryTabs })),
)
const QuestionList = lazy(() =>
  import('./components/QuestionList').then((mod) => ({ default: mod.QuestionList })),
)
const ContactSupport = lazy(() =>
  import('./components/ContactSupport').then((mod) => ({ default: mod.ContactSupport })),
)

const CategoryTabsSkeleton = lazy(() =>
  import('./components/CategoryTabs/CategoryTabsSkeleton').then((mod) => ({
    default: mod.CategoryTabsSkeleton,
  })),
)
const ContactSupportSkeleton = lazy(() =>
  import('./components/ContactSupport/ContactSupportSkeleton').then((mod) => ({
    default: mod.ContactSupportSkeleton,
  })),
)

export const FAQ: FC = () => {
  const [imageLoaded, setImageLoaded] = useState(false)
  const { searchTerm, setSearchTerm, activeCategory, setActiveCategory, filteredQuestions } =
