  ...
  try {
    ...
    const empresasDetalhadas: EmpresaDetalhada[] = empresasRaw.map((empresa: any) => {
      ...
      const dividendosEmpresa = dividendosData.find(
        (div: any) => div.nomeEmpresa && div.nomeEmpresa.toLowerCase() === empresa.nome.toLowerCase(),
      )
      ...
      const codigosMapeados: Codigo[] = empresa.codigos.map((cod: any) => ({
        codigo: cod.codigo,
        derivativos:
          typeof cod.derivativos === 'string' ? cod.derivativos === 'true' : Boolean(cod.derivativos),
        preco: cod.preco || 0,
        variacao: cod.variacao || 0,
