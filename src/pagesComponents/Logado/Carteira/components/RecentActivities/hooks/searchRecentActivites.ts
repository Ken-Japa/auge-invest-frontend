...
export function useSearchRecentActivities() {
  ...
  const [recentRealActivities, setRecentRealActivities] = useState<RecentActivity[]>([])
  const [recentVirtualActivities, setRecentVirtualActivities] = useState<RecentActivity[]>([])
  ...
  const fetchTransactionsForWallets = useCallback(async (wallets: Wallet[]) => {
    ...
    for (const wallet of wallets) {
      ...
      for (const position of positions) {
        const transactionsResponse = await api.wallet.getTransactionsByPositionId(position._id)
  ...
  const fetchRecentActivities = useCallback(async () => {
    ...
    try {
      ...
      const virtualActivities = await fetchTransactionsForWallets(virtualWallets)
