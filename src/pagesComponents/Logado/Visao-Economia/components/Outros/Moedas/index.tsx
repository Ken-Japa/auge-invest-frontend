...
export const Moedas = () => {
  ...
  const leftCurrencies = mockCurrencies.filter((c) => c.id !== selectedCurrency.id).slice(0, 3)
  ...
  return (
    <SuspenseWrapper fallback={<ContentSkeleton height={240} />}>
      <MoedasContainer>
        ...
        <MainCurrency>
          ...
          <MainCurrencyValue>R$ {formatCurrencyValue(selectedCurrency.value)}</MainCurrencyValue>
          <MainCurrencyVariation $isPositive={selectedCurrency.variation >= 0}>
            ...
            {selectedCurrency.variation >= 0 ? '+' : ''}
            {selectedCurrency.variation}%
          </MainCurrencyVariation>
        </MainCurrency>
