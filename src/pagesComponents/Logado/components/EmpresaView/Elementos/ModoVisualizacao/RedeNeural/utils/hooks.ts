  ...
  const handleSelectIndustry = useCallback(
    (industryId: string) => {
      if (networkRef.current) {
        ...
        const selectedIndustry = industriesForDropdown.find((ind) => ind.id === industryId)
        ...
        if (selectedIndustry) {
          ...
          const industrySegments = segmentsForDropdown.find((seg) => seg.industryId === industryId)
          if (industrySegments) {
            industrySegments.segments.forEach((segment) => {
              nodesToSelect.push(segment.id)
              // Find companies within this segment
              graphData.nodes.forEach((node) => {
                if (node.id.startsWith('empresa-') && node.segment === segment.label) {
    ...
    [networkRef, industriesForDropdown, segmentsForDropdown, setSelectedNodePath, graphData.nodes],
  )
  ...
  const handleSelectSegment = useCallback(
    (segmentId: string) => {
      if (networkRef.current) {
        ...
        graphData.nodes.forEach((node) => {
          if (node.id.startsWith('empresa-') && node.segment && `segmento-${node.segment}` === segmentId) {
            nodesToSelect.push(node.id)
          }
        })
        networkRef.current.selectNodes(nodesToSelect)
