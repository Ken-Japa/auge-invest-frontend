...
import { formatDate, parseDateDividends as parseDate } from '@/components/Helpers/Formatters/formatters'
import { api } from '@/services/api'
import { BDRDividendItem as Dividend, BDRDividendResponseData as BDRDividendResponse } from '@/services/api'
  ...
  useEffect(() => {
    const calculateSummary = () => {
      ...
      const values = dividends.map((div) => {
        const value = typeof div.valor === 'string' ? parseFloat(div.valor.replace(',', '.')) : div.valor
