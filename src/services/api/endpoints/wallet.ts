...
class WalletApiService extends BaseApiService {
  ...
  public async createWallet(payload: CreateWalletPayload): Promise<Wallet> {
    try {
      const response = await this.post<Wallet>(API_ENDPOINTS.WALLET.CREATE, payload)
      return response
    } catch (error) {
  ...
  public async updateWallet(id: string, payload: UpdateWalletPayload): Promise<Wallet> {
    try {
      const response = await this.put<Wallet>(`${API_ENDPOINTS.WALLET.UPDATE}/${id}`, payload)
      return response
    } catch (error) {
  ...
  public async getWalletPosition(id: string): Promise<WalletTransactions> {
    try {
      const response = await this.get<WalletTransactions>(`${API_ENDPOINTS.WALLET.GET_WALLET_POSITION}/${id}`)
      return response
    } catch (error) {
  ...
  public async getUserWallets(userId: string): Promise<Wallet[]> {
    try {
      const response = await this.get<Wallet[]>(`${API_ENDPOINTS.WALLET.GET_USER_WALLETS}/${userId}`)
      return response
    } catch (error) {
  ...
  public async getWalletById(id: string): Promise<Wallet> {
    try {
      const response = await this.get<Wallet>(`${API_ENDPOINTS.WALLET.GET_WALLET}/${id}`)
      return response
    } catch (error) {
      throw handleApiError(error, ErrorCode.WALLET_NOT_FOUND)
    }
  ...
  public async createTransaction(payload: CreateTransactionPayload): Promise<Transaction> {
    try {
      const response = await this.post<Transaction>(API_ENDPOINTS.TRANSACTION.CREATE, payload)
      return response
    } catch (error) {
  ...
  public async deleteTransaction(transactionId: string): Promise<void> {
    try {
      await this.delete<void>(`${API_ENDPOINTS.TRANSACTION.DELETE}/${transactionId}`)
    } catch (error) {
  ...
  public async getTransaction(transactionId: string): Promise<Transaction> {
    try {
      const response = await this.get<Transaction>(`${API_ENDPOINTS.TRANSACTION.GET}/${transactionId}`)
      return response
    } catch (error) {
