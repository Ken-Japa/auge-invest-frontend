...
class UserApiService extends BaseApiService {
  createUser = async (userData: Partial<User>): Promise<User> => {
    try {
      const response = await this.post<User>(API_ENDPOINTS.USER.CREATE, userData)
      return response
    } catch (error) {
  ...
  getUserById = async (userId: string): Promise<User> => {
    try {
      const response = await this.get<User>(`${API_ENDPOINTS.USER.READ}/${userId}`)
      return response
    } catch (error) {
  ...
  updateUser = async (userId: string, userData: Partial<User>): Promise<User> => {
    try {
      const response = await this.put<User>(`${API_ENDPOINTS.USER.UPDATE}/${userId}`, userData)
      return response
    } catch (error) {
  ...
  updatePassword = async (userId: string, oldPassword: string, newPassword: string): Promise<void> => {
    try {
      await this.put<ApiSuccessResponse<void>>(`${API_ENDPOINTS.USER.UPDATE_PASSWORD}/${userId}`, {
        oldPassword,
        newPassword,
      })
    } catch (error) {
      console.error(`Erro ao atualizar a senha para o usuário: ${userId}:`, error)
