export const formatCNPJ = (cnpj: string): string => {
  ...
  if (numericCNPJ.length === 14) {
    return `${numericCNPJ.slice(0, 2)}.${numericCNPJ.slice(
      2,
      5,
    )}.${numericCNPJ.slice(5, 8)}/${numericCNPJ.slice(8, 12)}-${numericCNPJ.slice(12)}`
  } else if (numericCNPJ.length === 13) {
    return `${numericCNPJ.slice(0, 2)}.${numericCNPJ.slice(
      2,
      5,
    )}.${numericCNPJ.slice(5, 8)}/${numericCNPJ.slice(8, 11)}-${numericCNPJ.slice(11)}`
  } else {
    return cnpj
  }
}
...
export const formatLink = (url: string | undefined, displayText = 'Link'): string => {
  if (!url) return 'N/A'
