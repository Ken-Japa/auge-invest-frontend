...
async function handleGoogleLogin(token: string) {
  try {
    const response = await fetch(getFullEndpointUrl(API_ENDPOINTS.AUTH.GOOGLE_LOGIN), {
      method: 'POST',
      headers: {
  ...
  providers: [
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          const response = await fetch(getFullEndpointUrl(API_ENDPOINTS.AUTH.LOGIN), {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (!response.ok) {
            console.error('Credentials login API response not OK.', response.status)
            return null
          }
    ...
    async signIn({ user, account }) {
      ...
      if (account?.provider === 'google' && account?.id_token) {
        try {
          ...
          console.error('Google authentication error in signIn callback:', error)
